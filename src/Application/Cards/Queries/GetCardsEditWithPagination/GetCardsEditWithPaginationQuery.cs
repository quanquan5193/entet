...
{
    public class GetCardsEditWithPaginationQuery : IRequest<PaginatedList<CardDto>>
    {
    ...
    public class GetCardsEditWithPaginationQueryHandler : IRequestHandler<GetCardsEditWithPaginationQuery, PaginatedList<CardDto>>
    {
        ...
        public GetCardsEditWithPaginationQueryHandler(IApplicationDbContext context, IMapper mapper, IIdentityService identityService, IConfiguration configuration)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<PaginatedList<CardDto>> Handle(GetCardsEditWithPaginationQuery request, CancellationToken cancellationToken)
        {
            int pageNumber = 1;
            int.TryParse(request.PageNumber, out pageNumber);
            IQueryable<CardDto> query = _identityService.GetCardsWithUserQueryable().Where(n => n.Store.IsActive && n.Company.IsActive);
