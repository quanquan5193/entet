...
namespace mrs.Application.Members.Commands.RegisterMember
{
    public class RegisterMemberCommandValidator : AbstractValidator<RegisterMemberCommand>
    {
        ...
        public RegisterMemberCommandValidator(IApplicationDbContext context, ICurrentUserService currentUserService, IIdentityService identityService)
        {
            _context = context;
        ...
        {
            return await _context.Cards.AnyAsync(x => x.MemberNo.Equals(memberNo) && x.Status == CardStatus.Unissued && !x.IsDeleted);
        ...
        public async Task<bool> CheckMemberNoStore(string memberNo, CancellationToken cancellationToken)
        {
            ...
            return await _context.Cards.AnyAsync(x => x.MemberNo.Equals(memberNo) && x.StoreId == store.Id && !x.IsDeleted);
